...
Due to the nature of the BT dialogue system, sometimes prompt nodes (blue nodes) that have a conditional on them require an empty response node (yellow) to follow it. Empty response nodes just get followed by the conversation system so you won't see the empty response in the actual conversation - think of it as the conversation logic process. I've noticed this isn't always the case though and haven't narrowed down the logic why this happens (or I've forgotten since I originally wrote ConverseTek). My recommendation: try without the empty nodes and responses first - if you get issues with the condition or action not triggering then put in an empty prompt node and response node.
